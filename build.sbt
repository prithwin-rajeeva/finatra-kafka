...
lazy val rootProject = project
  .in(file("."))
  .settings(name := "finatra kafka",
    organization := "in.prithw",
...
bashScriptExtraDefines ++= Seq("""addApp "-log.level=${LOG_LEVEL:-INFO}"""",
                               """addApp "-swagger.docs.endpoint=${SWAGGER_DOC_PATH:-/finatrakafka/docs}"""",
...
maintainer         in Docker := "Prithwin Rajeeva <prithwin@outlook.com>"
...
dockerLabels := Map(
  ...
  "owner_team" -> "Prithwin Rajeeva",
  "build_id"   -> Option(System.getProperty("build_id")).getOrElse("NA"),
  "base_image" -> dockerBaseImage.value
)

dockerEnvVars := Map("SERVICE_NAME" -> "finatra-sample-service", "SERVICE_TAGS" -> "vr,service")
...
// MicroSites
micrositeName                 := "finatra kafka"
micrositeBaseUrl              := "finatra-kafka"
micrositeDocumentationUrl     := "/finatra-kafka/docs"
micrositeAuthor               := "Prithwin Rajeeva"
